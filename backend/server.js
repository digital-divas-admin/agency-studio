...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Allow explicitly configured origins (e.g. specific staging URLs)
...
// Stricter limit for generation/editing endpoints (POST only)
// GET requests (status polling) use the general limiter (100/min) to avoid
// blocking async job polling for Qwen and Inpaint routes
...
// Portal routes (public, token-auth — placed under /api so agency resolution applies)
...
// Editing routes (same rate limit as generation)
app.use('/api/edit', generationLimiter, editingRoutes);

// Chat route (general rate limit is sufficient)
...
const server = app.listen(config.port, () => {
  ...
  logger.info(`RunPod dedicated URL: ${config.runpod.dedicatedUrl || 'NOT SET'}`);
  logger.info(`RunPod serverless URL: ${config.runpod.serverlessUrl || 'NOT SET'}`);
  logger.info(`RunPod API key: ${config.runpod.apiKey ? 'SET' : 'NOT SET'}`);
